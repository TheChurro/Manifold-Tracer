...
__kernel void trace(
  ...
  for (int i = 0; i < num_balls; i++) {
    ...
    // If the tangent is perpendicular to the ball.
      ...
      float y0 = (r - center_origin) / center_tangent * x0;
      float theta0 = atan2(y0, x0);
      if (theta0 < 0) theta0 += 2 * M_PI_F;
      float x1 = (-b + descriminant_sqrt) / (2 * a);
      float y1 = (r - center_origin) / center_tangent * x1;
  ...
  if (was_hit) {
    // float3 tangent_color = 0.5f + (0.05f + 0.2f * (2.0f - hit_normal.w)) * hit_normal.xyz;
    ...
    // color_mask = (2.0f * M_PI_F - hit_angle) / (2.0f * M_PI_F) * color_mask;
    // float hit_percent = fabs(1 - (M_PI_F / 2.0f - hit_angle) / (2.0f * M_PI_F));
    write_imagef(
      out_image,
      pixel,
